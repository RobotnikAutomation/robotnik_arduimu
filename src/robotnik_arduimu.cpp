...
*/
robotnik_arduimu_node::robotnik_arduimu_node( std::string port ) : desired_freq_(125) {

	// frame IMU   T:-944,AX:-5,AY:-12,AZ:-31,GX:2,GY:-1,GZ:0,MGX:-203,MGY:-307,MGZ:-348,MGH:117.47
    ...
    iIndexAverageZ=0;
    	...
    	// Create serial port
	serial= new SerialDevice(port.c_str(), ARDUIMU_DEFAULT_TRANSFERRATE, ARDUIMU_DEFAULT_PARITY, ARDUIMU_DEFAULT_DATA_SIZE); 
};
...
int robotnik_arduimu_node::Open(){
	...
	ROS_INFO("robotnik_arduimu_node::Open: serial port opened at %s", serial->GetDevice());
...
void robotnik_arduimu_node::InitState(){
			...
			if (bCalibrating == true) {
				...
				if (iCyclesStabilization>20) {
					...
					if (iCyclesCalibration == 20) {
						...
						bCalibrated_ = true;
						bCalibrating = false;
						ROS_INFO("Calibration done - gyro calibration daX:%d daY:%d daZ:%d", dgX_, dgY_, dgZ_);
	...
	if(count_errors_reading >= ARDUIMU_MAX_ERRORS_INIT){
		...
		ROS_ERROR("robotnik_arduimu_node::InitState: %d errors reading", count_errors_reading);
		SwitchToState(FAILURE_STATE);
...
void robotnik_arduimu_node::ReadyState(){
	...
	if ( (current_time - tArduImuReply).toSec() > ARDUIMU_TIMEOUT_COMM)  {
		ROS_ERROR("robotnik_arduimu_node::ReadyState: Timeout in communication with the device");
		tArduImuReply = ros::Time::now();	// Resets the timer
		iErrorType=ARDUIMU_ERROR_TIMEOUT;
		SwitchToState(FAILURE_STATE);
    } 
    if ( (current_time - tArduImuReply).toSec() > ARDUIMU_TIMEOUT_COMM/100)  {
		bOKReadings=false;
	}
}
...
void robotnik_arduimu_node::FailureState(){
	...
	if(recovery_cycles >= 100){ //Try to recover every 'second'
		ROS_INFO("robotnik_arduimu_node::FailureState: Trying to recover..");
		switch(iErrorType)	{
			case ARDUIMU_ERROR_OPENING://Try to recover
				ROS_ERROR("robotnik_arduimu_node::FailureState: Recovering from failure state (ARDUIMU_ERROR_OPENING.)");
				this->Close();
				usleep(timer);
				this->Open();
				SwitchToState(INIT_STATE);
			...
			case ARDUIMU_ERROR_TIMEOUT:
				ROS_ERROR("robotnik_arduimu_node::FailureState: Recovering from failure state (ARDUIMU_ERROR_TIMEOUT.)");
				this->Close();
				usleep(timer);
				this->Open();
				SwitchToState(INIT_STATE);
			...
			case ARDUIMU_ERROR_SERIALCOMM:
				ROS_ERROR("robotnik_arduimu_node::FailureState: Recovering from failure state (ARDUIMU_ERROR_SERIALCOMM.)");
				this->Close();
				usleep(timer);
				this->Open();
				SwitchToState(INIT_STATE);
...
int robotnik_arduimu_node::ReadControllerMsgs(){
	...
	if (serial->ReadPort(cReadBuffer, &read_bytes, 200)==ERROR) {
	    ROS_ERROR("robotnik_arduimu_node::ReadControllerMsgs: Error reading port");
	    return ERROR;
        }
	...
	while(read_bytes > 0){
		...
		if(ret == ERROR) return ERROR;
           ...
           if (serial->ReadPort(cReadBuffer, &read_bytes, 100)==ERROR) {
				//ROS_ERROR("robotnik_arduimu_node::ReadControllerMsgs: Error after 1st port read");
				//return ERROR;
                ret = OK;
           }
        }
...
//T:-964,AX:1739,AY:-62,AZ:7698,GX:4312,GY:-3573,GZ:-1459,MX:-307,MY:-2,MZ:319,MH:173.63,LT:0,LN:0,AT:0,CG:0,SG:0,FX:0,ST:0,TW:0,
	...
	if (i!=TOKENS_IMU) {
		ROS_ERROR("robotnik_arduimu_node::ProcessMsg: received unexpected number of tokens %d", i);
		return ERROR;
		bOKReadings=false;
		}
	...
	for (j=0;j<i;j++) {
		...
		// ROS_INFO("st1 %s   st2 %s", cReceivedSubTokens[0], cReceivedSubTokens[1]);
		// Processing of IMU tokens
		// frame IMU   T:-944,AX:-5,AY:-12,AZ:-31,GX:2,GY:-1,GZ:0,MGX:-203,MGY:-307,MGZ:-348,MGH:117.47
		if (i==TOKENS_IMU) {
			if(!strcmp(cReceivedSubTokens[0], "D")) temperature_ = atof(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "X")) {
				aX_ = atoi(cReceivedSubTokens[1]);
				aXF_[iIndexAverageX]=aX_; 
				iIndexAverageX++;
				if (iIndexAverageX>=NUM_SAMPLES_FOR_AVERAGE) iIndexAverageX=0;
			}
			if(!strcmp(cReceivedSubTokens[0], "Y")) {
				aY_ = atoi(cReceivedSubTokens[1]);
				aYF_[iIndexAverageY]=aY_; 
				iIndexAverageY++;
				if (iIndexAverageY>=NUM_SAMPLES_FOR_AVERAGE) iIndexAverageY=0;
			}
			if(!strcmp(cReceivedSubTokens[0], "Z")) {
				aZ_ = atoi(cReceivedSubTokens[1]);
				aYZ_[iIndexAverageZ]=aZ_;
				iIndexAverageZ++;
				if (iIndexAverageZ>=NUM_SAMPLES_FOR_AVERAGE) iIndexAverageZ=0;
			}
			if(!strcmp(cReceivedSubTokens[0], "A")) gX_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "B")) gY_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "C")) gZ_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "R")) mX_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "S")) mY_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "T")) mZ_ = atoi(cReceivedSubTokens[1]);
			// if(!strcmp(cReceivedSubTokens[0], "MH")) mH_ = atof(cReceivedSubTokens[1]); // float
			// TT contains measured time in ms between succesive sends (about 5 - 6 ms)
			if(!strcmp(cReceivedSubTokens[0], "M")) mH_ = atoi(cReceivedSubTokens[1]); 
		}
	}
			
	// Publish YAW from accelerometer
	float aXTemp=0.0;
	float aYTemp=0.0;
	float aZTemp=0.0;
	for (int i=0;i<NUM_SAMPLES_FOR_AVERAGE; i++){
		aXTemp=aXTemp+aXF_[i]; 
		aYTemp=aYTemp+aYF_[i]; 
		aZTemp=aZTemp+aYZ_[i];
	}	
	aVX_=aXTemp/NUM_SAMPLES_FOR_AVERAGE; 
	aVY_=aYTemp/NUM_SAMPLES_FOR_AVERAGE;  
	aVZ_=aZTemp/NUM_SAMPLES_FOR_AVERAGE;
	
	//ROS_INFO(" X:%f,Y:%F,Z:%f",aVX_,aVY_,aVZ_);
		/* 
		if (i==TOKENS_IMU) {
			if(!strcmp(cReceivedSubTokens[0], "T")) temperature_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "AX")) aX_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "AY")) aY_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "AZ")) aZ_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "GX")) gX_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "GY")) gY_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "GZ")) gZ_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "MX")) mX_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "MY")) mY_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "MZ")) mZ_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "MH")) mH_ = atof(cReceivedSubTokens[1]); // float				
			}
		// Processing of GPS tokens
	        // frame GPS   LAT:0,LON:0,ALT:0,COG:0,SOG:0,FIX:0,SAT:0,TOW:0, 
		if (i==TOKENS_GPS) {
			if(!strcmp(cReceivedSubTokens[0], "LT")) latitude_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "LN")) longitude_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "AT")) altitude_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "CG")) cog_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "SG")) sog_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "FX")) fix_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "ST")) sat_ = atoi(cReceivedSubTokens[1]);
			if(!strcmp(cReceivedSubTokens[0], "TW")) tow_ = atoi(cReceivedSubTokens[1]);
			}
                */
	bOKReadings=true;
	return OK;
...
// CALLBACKS
// Service Calibrate
bool robotnik_arduimu_node::srvCallback_Calibrate(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
	bCalibrated_ = false;
	SwitchToState(INIT_STATE);
	return true;
...
bool robotnik_arduimu_node::srvCallback_MagYawReset(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
...
int main(int argc, char** argv)
{
	...
	// Advertise services
	srv_Calibrate = n.advertiseService("imu/Calibrate", &robotnik_arduimu_node::srvCallback_Calibrate, rbk_arduimu);
       ...
       // Services
        ...
        // Advertise services
        srv_Calibrate = n.advertiseService("imu/MYReset", &robotnik_arduimu_node::srvCallback_MagYawReset, rbk_arduimu);

	
	// Publishing
	ros::Publisher imu_pub = n.advertise<sensor_msgs::Imu>("imu/data_raw", 50);
	ros::Publisher mag_pub = n.advertise<geometry_msgs::Vector3Stamped>("imu/mag", 50);
    ros::Publisher yaw_mag_pub = n.advertise<std_msgs::Float32>("imu/yaw_mag", 50);
    ros::Publisher temp_pub = n.advertise<std_msgs::Float32>("imu/temperature", 50);
    // Yaw from accelerometter
    ros::Publisher yaw_acc_pub = n.advertise<std_msgs::Float32>("imu/yaw_acc", 50);
    
    ros::Publisher ok_pub = n.advertise<std_msgs::Bool>("imu/data_ok", 50);

	tf::TransformBroadcaster tf_broadcaster;
	
	if( rbk_arduimu->Open()==OK ) ROS_INFO("Connected to robotnik_arduimu_node component");
	else
	{
	...
	while(n.ok())
	{
		...
		// For Acceleration calibration purposes
		//double dModule= sqrt(lin_acc.x*lin_acc.x+lin_acc.y*lin_acc.y+lin_acc.z*lin_acc.z);
		//ROS_INFO("Module = %f",dModule);
		...
		// ZXY fallo rotx90 rotz180
 		float mxc = (float) ((rbk_arduimu->mX_ * 8.0 / 32768.0) - x_offset) / x_gain;
        float myc = (float) ((rbk_arduimu->mY_ * 8.0 / 32768.0) - y_offset) / y_gain;
        float mzc = (float) ((rbk_arduimu->mZ_ * 8.0 / 32768.0) - z_offset) / z_gain;
        MagMsg.vector.x = mxc;
        MagMsg.vector.y = myc;
        MagMsg.vector.z = mzc;
		...
		// ROS_INFO("mX_=%d mY_=%d mZ_=%d", rbk_arduimu->mX_, rbk_arduimu->mY_, rbk_arduimu->mZ_ );
		...
		if (rbk_arduimu->yaw_mag_ > 3.1415926535) rbk_arduimu->yaw_mag_ -= 2.0 * 3.1415926535;
		if (rbk_arduimu->yaw_mag_ < -3.1415926535) rbk_arduimu->yaw_mag_ += 2.0 * 3.1415926535; 
		...
		// Publish data OK
		std_msgs::Bool OKData;
		OKData.data=rbk_arduimu->bOKReadings;
		ok_pub.publish(OKData);		
		
		std_msgs::Float32 yaw_acc_msg;
		yaw_acc_msg.data =atan2(-rbk_arduimu->aVX_,rbk_arduimu->aVY_)*180.0/3.1415;
		if (iIndexP>10) {
			yaw_acc_pub.publish(yaw_acc_msg);
			iIndexP=0;
		}
        iIndexP++;
        ...
        if (iIndexT>100) {
            std_msgs::Float32 temper_msg;
            temper_msg.data=(float)rbk_arduimu->temperature_/100.0;
            temp_pub.publish(temper_msg);
            iIndexT=0;
        }
        iIndexT++;
