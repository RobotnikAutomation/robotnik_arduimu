...
#define ERROR_CRC					1
#define ARDUIMU_SERIAL_DELAY				10000		//! us between serial transmisions to the controller
#define ARDUIMU_MAX_ERRORS_INIT				1000		//! Max number of consecutive communications errors in INIT state
#define NUM_SAMPLES_FOR_AVERAGE				100.0			//! NUM Samples for Average Accelerometter
...
public:
	...
	// If Last value don't to much time change error
	int aXLast_;
	
	//! Raw Data for filtering
	int aXF_[1000], aYF_[1000], aYZ_[1000];   // Accelerations
	//! Average Accelerations
    float aVX_, aVY_, aVZ_;   // Accelerations
    //! Index for filtering
    int iIndexAverageX;
    int iIndexAverageY;
    int iIndexAverageZ;
...
public:
	...
	bool srvCallback_Calibrate(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response);
	//! Service Callback - Reset Magnetic Yaw
	bool srvCallback_MagYawReset(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response); 
